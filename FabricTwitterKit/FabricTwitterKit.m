...
{
    TWTRSession *session = [self getSession];
...
RCT_EXPORT_METHOD(fetchProfile:(RCTResponseSenderBlock)callback)
{
    ...
    TWTRSession *lastSession = [self getSession];
...
RCT_EXPORT_METHOD(sendTwitterRequest:(NSDictionary *)options :(RCTResponseSenderBlock)callback)
{
    TWTRSession *lastSession = [self getSession];
    ...
    if (request) {
        ...
        NSLog(@"Error: %@", clientError);
        callback(@[[clientError localizedDescription]]);
    }
}
